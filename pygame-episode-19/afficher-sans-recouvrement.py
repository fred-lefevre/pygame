#
# Initiation à Pygame - Épisode 19 - Afficher des rectangles sans recouvrement
#
# https://kreatuto.info
#
...
pygame.display.set_caption("Épisode 19")
...
rectangles = list()
while len(rectangles) < 5:
    candidat = pygame.Rect(20 + random.randint(0, 380), 20 + random.randint(0, 220), LARGEUR_REC, HAUTEUR_REC)
    sans_recouvrement = True
    for rect in rectangles:
        if rect.colliderect(candidat):
            sans_recouvrement = False
    if sans_recouvrement:
        rectangles.append(candidat)
